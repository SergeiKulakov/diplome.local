<?php
  ...
  class QuestionsModel extends DataBase {
    ...
    public static function viewAllQuestions() {
      $sql = "SELECT q.*, c.title, c.id cat_id, s.title status_title FROM question_answer q JOIN categories c ON q.category_id = c.id JOIN status s ON s.id = q.status_id ORDER BY category_id";
    ...
    public static function viewEmptyQuestions() {
      $sql = "SELECT q.*, c.title, c.id cat_id FROM question_answer q LEFT JOIN categories c ON q.category_id = c.id WHERE q.status_id = 1 ORDER BY q.receipt_date";
      $sth = self::DB()->prepare($sql);
      $sth->execute();
      $result = $sth->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public static function viewVisibleQuestions() {
      $sql = "SELECT * FROM question_answer WHERE status_id = 2";
      $sth = self::DB()->prepare($sql);
      $sth->execute();
      $result = $sth->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public static function viewQuestion($id) {
      $sql = "SELECT q.*, c.title title_cat, c.id id_cat, s.id id_status, s.title title_status FROM question_answer q JOIN categories c ON c.id = q.category_id JOIN status s ON s.id = q.status_id WHERE q.id = ?";
      $sth = self::DB()->prepare($sql);
    ...
    public static function questionsInTheme($id) {
      ...
      $sql = "SELECT  q.*, s.title FROM question_answer q JOIN status s ON s.id = q. status_id WHERE q.category_id = ?";
      $sth = self::DB()->prepare($sql);
      $sth->bindParam(1, $id, \PDO::PARAM_INT);
      $sth->execute();
      $result = $sth->fetchAll(\PDO::FETCH_ASSOC);
